...
Technological innovation has long been viewed as the main driver of economic growth. It is clear that technological innovations come in many different types, including in their scale, scope, and type of impact. Recent literature on endogenous growth has included heterogeneity in innovation. In this paper a model is presented in which firms endogenously choose between two different types of innovation: abrupt (major) and incremental (minor). This feature allows the model to explain a larger range of behavior, and has a number of interesting predictions. In this work we take a first step toward applying the model; a few key parameters are estimated using data on patent citation distributions.

Our theoretical model directly builds off of recent literature in endogenous growth theory, particularly the work of Ufuk Akcigit. The recent work has taken into account the idea of R\&D heterogeneity (Akcigit and Kerr, 2016) and diminishing returns of incremental innovation (Acemoglu, Akcigit and Celik, 2014). These works use a model where some special (abrupt) innovations result in the creation of a new "technology cluster", which then contains a number of follow-up (incremental) innovations until the whole technology cluster is finally replaced by another abrupt innovation. Our model utilizes this same framework. However, in none of the previous models is the heterogeneous innovation decision endogenous; firms simply invest in general R\&D, which then results in different types of innovations according to an exogenous probability distribution. In our model firms can choose to invest in two different types of R\&D, one of which results in abrupt innovations while the other results in incremental innovations. Additionally, to our knowledge, there is no model that accounts for the possibility of incumbent firms investing in abrupt innovations, which eliminates the possibility of a temporal dimension of the innovation decision.
...
\begin{table}[htbp] \centering \captionsetup{width=5.8in}
	...
	\toprule
	MLE & \multicolumn{1}{c}{$\alpha$} & \multicolumn{1}{c} {$\frac{\lambda_{abr, int} + \tau}{\lambda_{inc,0}}$} \\ 
	\midrule
	Results & 10.06 & 7.39\\
...
\begin{figure}[htb]\centering \captionsetup{width=5.8in}
    \fbox{\resizebox{3.0in}{2.0in}{\includegraphics{CBFirms.jpeg}}}
    \caption{\label{fig:CBFirms}\textit{Evolution of the ratio of sprinter firms in the economy.}}
\end{figure}

\begin{figure}[htb]\centering \captionsetup{width=5.8in}
    \fbox{\resizebox{3.0in}{2.0in}{\includegraphics{RD_spending_ratio.jpeg}}}
...
\begin{figure}[htb]\centering \captionsetup{width=5.8in}
    \fbox{\resizebox{3.0in}{2.0in}{\includegraphics{GMM_MLE.png}}}
